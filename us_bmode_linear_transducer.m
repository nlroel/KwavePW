...
if RUN_SIMULATION
    ...
    save us_bmode_sensor_data sensor_data;
    
else
    
    % load the scan lines from disk
    load us_bmode_sensor_data;
    
end

das_process = DAS(kgrid, transducer, c0);
das_process = das_process.load(sensor_data);
scan_lines = das_process.get_scan_lines(transducer.number_elements);
...
scale_factor = 2;
scan_lines_fund = interp2(1:kgrid.Nt, (1:transducer.number_elements).', scan_lines_fund, 1:kgrid.Nt, (1:1/scale_factor:transducer.number_elements).');
...
figure;
imagesc((transducer.position(2)-1:transducer.position(2)+transducer.number_elements * transducer.element_width-1) * dy * 1e3, (0:Nx_tot-1) * dx * 1e3, sound_speed_map(:, transducer.position(2):transducer.position(2)+transducer.number_elements * transducer.element_width, Nz/2));
axis image;
colormap(gray);
set(gca, 'YLim', [5, 40]);
...
imagesc((transducer.position(2)-1)*dy*1e3+horz_axis, r * 1e3, scan_lines_fund.');
axis image;
colormap(gray);
set(gca, 'YLim', [5, 40]);
title('B-mode Image');
